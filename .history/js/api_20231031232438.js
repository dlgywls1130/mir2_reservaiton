...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const headerLoginButton = document.querySelector('.login_btn .view_more');
    const loginInfoSpan = document.querySelector('.login_btn span');
    let rawInput = "";
    ...
    loginButton.addEventListener('click', function() {
        ...
        .then(data => {
            if (data.isPrebooked) {
                ...
                location.reload();  // 페이지를 새로고침하여 해당 섹션으로 이동시킵니다.
            }
        })
        .catch(error => {
            console.error('API 호출 중 에러 발생:', error);
