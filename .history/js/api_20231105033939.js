...
function initializeChances() {
    if (isLoggedIn) {
        // 로그인 후 기본적으로 남은 기회를 4회로 설정합니다.
        localStorage.setItem('remainingChances', 4);
        updateRemainingChances(4);
    } else {
        // 로그인 전에는 남은 기회를 0으로 설정합니다.
    ...
    fetchRemainingChances()
        .then(chancesLeft => {
            ...
            let chancesLeft = parseInt(localStorage.getItem('remainingChances'), 10);
                if (isNaN(chancesLeft)) {
                    console.error('남은 기회를 가져오는데 실패했습니다. 기본값으로 설정합니다.');
                    chancesLeft = 3; // 기본값으로 3을 설정
                }
