...
function onPageLoad() {
    // 로컬 스토리지에서 로그인 상태와 계정 ID를 확인합니다.
    const token = localStorage.getItem('token');
    const accountId = localStorage.getItem('accountId');
  
    // 로그인 상태를 확인하고 게임 기록을 불러옵니다.
    if (token && accountId) {
      isLoggedIn = true;
      fetchGameHistory(accountId); // 로그인한 경우 게임 기록을 불러옵니다.
    } else {
      isLoggedIn = false;
      chances = 0; // 로그인하지 않은 경우 기회를 0으로 설정합니다.
      updateChancesDisplay(); // 화면에 횟수를 업데이트합니다.
    }
  }

  // 게임 기록을 불러오고 남은 횟수를 업데이트하는 함수입니다.
function fetchGameHistory(accountId) {
    const date = new Date().toISOString().split('T')[0]; // 오늘 날짜를 YYYY-MM-DD 형식으로 변환
    ...
    fetch(`https://mir2red.com/api/rulet/${accountId}/${today}`, {
      ...
      .then(data => {
        // 서버로부터 받은 데이터를 바탕으로 남은 횟수를 계산합니다.
        const gamesPlayedToday = data.length;
        chances = 4 - gamesPlayedToday;
        updateChancesDisplay(); // 남은 횟수를 업데이트하는 함수를 호출합니다.
      })
      .catch(error => {
        console.error('게임 이력을 불러오는 중 에러가 발생했습니다:', error);
        alert(error.message); // 사용자에게 에러 알림
      });
    }


  // 남은 기회를 화면에 표시하는 함수입니다.
function updateChancesDisplay() {
    const chancesDisplay = document.querySelector('.rollet_span span');
    chancesDisplay.textContent = `기회 ${chances}회`;
  }
