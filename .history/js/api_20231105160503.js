...
document.addEventListener("DOMContentLoaded", function() {
    ...
    loginButton.addEventListener('click', function() {
        ...
        .then(response => {
            if (response.ok) { // 200-299 상태 코드를 확인합니다.
                return response.json().then(body => ({ status: response.status, body: body }));
            } else {
                // JSON이 아닌 응답 본문을 처리합니다. text() 대신 json()을 사용해야 할 경우가 있을 수 있습니다.
                return response.text().then(text => {
                    let error = { status: response.status, message: text };
                    try {
                        // 만약 응답이 JSON이라면, 파싱을 시도합니다.
                        error.message = JSON.parse(text);
                    } catch(e) {
                        // 파싱에 실패하면 원본 텍스트 메시지를 유지합니다.
                    }
                    throw error; // 오류를 던져 catch 블록으로 전달합니다.
                });
            }
        })
        .then(data => {
        ...
        .catch(error => {
            if (error.status === 401) {
                // 401 Unauthorized 처리를 합니다.
                alert('로그인 정보가 올바르지 않습니다.');
            } else {
                // 기타 에러 처리를 합니다.
                console.error('API 호출 중 에러 발생:', error.message || error);
                alert('서버 에러가 발생했습니다.');
            }
        });
    });
