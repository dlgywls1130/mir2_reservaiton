...
document.addEventListener("DOMContentLoaded", function() {
    ...
    updateLoginStatus(localStorage.getItem('isLoggedIn') === 'true');
    ...
    headerLoginButton.addEventListener('click', function() {
        if (isLoggedIn) {
            // 로그아웃 처리를 updateLoginStatus 함수를 이용하여 일관성 있게 처리
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    shareUrlButton.addEventListener('click', function() {
        ...
        // 이벤트 참여 API 호출
        const requestBody = {
            tel: localStorage.getItem('appropriateKeyForPhoneNumber'), // 예시로 적절한 키 이름으로 변경
        ...
        .then(response => {
            if (!response.ok) {
                return response.json().then(data => {
                    // 에러 메시지가 JSON 응답 본문에 포함되어 있다면 data를 사용할 수 있습니다.
