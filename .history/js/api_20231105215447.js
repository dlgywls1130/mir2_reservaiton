...
function initializeGameHistory(accountId, chances) {
    const token = localStorage.getItem('token');
    ...
    fetch('https://mir2red.com/api/rulet/initialize', { // 이 URL은 실제 서버의 엔드포인트 URL로 변경해야 합니다.
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
    ...
    .then(response => {
        if (response.ok) {
            ...
            // 여기서 HTTP 에러 처리를 해야 합니다.
            throw new Error(`Error: ${response.statusText}`);
        }
    })
    .then(data => {
        console.log('초기 게임 기회가 성공적으로 설정되었습니다.', data);
        // 여기서 추가적인 성공 처리를 할 수 있습니다.
    })
    .catch(error => {
        console.error('initializeGameHistory 호출 중 에러 발생:', error);
