...
function fetchGameHistory(accountId) {
    ...
    fetch(`https://mir2red.com/api/rulet/${accountId}/${date}`, { // 실제 서버 URL로 변경해야 합니다.
        method: 'GET',
        headers: {
    ...
    .then(data => {
        if (data && data.remainingChances !== undefined) {
            setChances(data.remainingChances); // 서버로부터 받은 기회로 업데이트
        } else {
            // 데이터 형식이 잘못된 경우 처리
            throw new Error('게임 이력 데이터 형식이 올바르지 않습니다.');
        }
    })
    .catch(error => {
        console.error('게임 이력 호출 중 에러 발생:', error.message);
        alert(error.message);
    });
}
...
function setChances(num) {
    chances = num;
    // UI에 남은 기회 업데이트
    document.getElementById('remainingChances').textContent = num; // 수정된 부분
}
