...
document.addEventListener("DOMContentLoaded", function() {
    const shareUrlInput = document.querySelector('input[type="text"]');
    const submitShareUrlButton = document.querySelector('.url_input_share');

    // 공유 URL 버튼 클릭 이벤트
    submitShareUrlButton.addEventListener('click', function() {
        // 로그인 여부를 확인합니다.
        if (!isLoggedIn) {
            // 로그인하지 않았을 경우 confirm을 사용하여 사용자의 의도를 확인합니다.
            if (confirm("로그인 후 진행할 수 있습니다. 지금 로그인 하시겠습니까?")) {
                // 확인을 클릭했을 경우 로그인 모달을 표시합니다.
                modals[0].style.display = 'block'; 
            }
            return; // 함수를 여기서 종료하여 더 이상 진행되지 않도록 합니다.
        }
        ...
        fetch('https://mir2red.com/api/event-second/create', {
            ...
            body: JSON.stringify({ url: shareUrlInput.value }) // 입력된 URL을 서버로 보냅니다.
        })
        .then(response => {
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
