...
// 공유 URL API 호출
document.addEventListener("DOMContentLoaded", function() {
    const shareUrlInput1 = document.querySelector('.url-share-btn_input input'); // 공유 URL 입력 필드
    const shareUrlButton1 = document.querySelector('.url-share-btn'); // 공유 URL 제출 버튼
    
    shareUrlButton1.addEventListener('click', function() {
        ...
        // 그 다음 URL 입력 확인
        const sharedUrl1 = shareUrlInput1.value.trim();
        if (!sharedUrl1) {
            alert("공유 URL을 입력해주세요.");
            return;
        }

        // 로그인 되어 있고 URL 입력됐다면 공유 URL 제출 함수 호출
        submitShareUrl1(accountId, sharedUrl1, token);
    });
});

function submitShareUrl1(accountId, sharedUrl1, token) {
    fetch('https://mir2red.com/api/event-first/create', {
        ...
        body: JSON.stringify({ accountId: accountId, sharedUrl1: sharedUrl1 })
    })
    .then(response => {
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
...
// 공유 URL API 호출
document.addEventListener("DOMContentLoaded", function() {
    const shareUrlInput = document.querySelector('.rollet_share_link input'); // 공유 URL 입력 필드
    const shareUrlButton = document.querySelector('.rollet_share_link_btn'); // 공유 URL 제출 버튼
    ...
    shareUrlButton.addEventListener('click', function() {
        ...
        // 로그인 되어 있고 URL 입력됐다면 공유 URL 제출 함수 호출
        submitShareUrl(accountId, sharedUrl, token);
    });
});

function submitShareUrl(accountId, sharedUrl, token) {
    fetch('https://mir2red.com/api/event-second/create', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
