...
function updateGameHistory() {
    ...
    .then(data => {
        // 여기서 data는 게임 이력의 배열이라고 가정합니다.
        // 예를 들면, [{ isSuccess: true }, { isSuccess: false }, ...] 형식입니다.
        let wins = data.filter(result => result.isSuccess).length;
        let losses = data.length - wins;

        // 승패 전적을 화면에 표시
        document.querySelector('.rollet-score span').textContent = `${wins}승 ${losses}패`;
    })
    .catch(error => {
        console.error('API 호출 중 에러 발생:', error);
        alert(error.message); // 사용자에게 에러 알림
