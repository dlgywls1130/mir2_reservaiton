...
document.addEventListener('DOMContentLoaded', function() {
  let selectedOption = null;  // '묵', '찌', '빠' 중 사용자가 선택한 값을 저장
  const rolletGamePan = document.querySelector('.rollet_game_pan');
  
  // 사용자의 선택을 처리하는 함수
  document.querySelector('.step_btn_wrap').addEventListener('click', function(event) {
      const target = event.target;
      if (target.matches('.someButtonSelectorFor묵')) {
          selectedOption = '묵';
      } else if (target.matches('.someButtonSelectorFor찌')) {
          selectedOption = '찌';
      } else if (target.matches('.someButtonSelectorFor빠')) {
          selectedOption = '빠';
      }
  });
